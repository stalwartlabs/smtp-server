...
use ahash::AHashMap;
use mail_auth::{
    common::crypto::{Algorithm, Ed25519Key, HashAlgorithm, RsaKey, SigningKey},
...
impl Config {
    pub fn parse_mail_auth(&self, ctx: &ConfigContext) -> super::Result<MailAuthConfig> {
        ...
        Ok(MailAuthConfig {
            dkim: DkimAuthConfig {
                verify: self
                    .parse_if_block("auth.dkim.verify", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(VerifyStrategy::Relaxed)),
                sign: self
                    .parse_if_block::<Vec<String>>("auth.dkim.sign", ctx, &envelope_keys)?
                    .unwrap_or_default()
                    .map_if_block(&signers, "auth.dkim.sign", "signature")?,
                report_request: self
                    .parse_if_block("auth.dkim.reporting.request", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(true)),
                report_send: self
                    .parse_if_block("auth.dkim.reporting.send-rate", ctx, &envelope_keys)?
                    .unwrap_or_default(),
                report_analyze: self
                    .parse_if_block("auth.dkim.reporting.analyze", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(true)),
            },
            arc: ArcAuthConfig {
                verify: self
                    .parse_if_block("auth.arc.verify", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(VerifyStrategy::Relaxed)),
                seal: self
                    .parse_if_block::<Option<String>>("auth.arc.seal", ctx, &envelope_keys)?
                    .unwrap_or_default()
                    .map_if_block(&sealers, "auth.arc.seal", "signature")?,
            },
            spf: SpfAuthConfig {
                verify_ehlo: self
                    .parse_if_block("auth.spf.verify.ehlo", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(VerifyStrategy::Relaxed)),
                verify_mail_from: self
                    .parse_if_block("auth.spf.verify.mail-from", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(VerifyStrategy::Relaxed)),
                report_send: self
                    .parse_if_block("auth.spf.reporting.send-rate", ctx, &envelope_keys)?
                    .unwrap_or_default(),
                report_analyze: self
                    .parse_if_block("auth.spf.reporting.analyze", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(true)),
            },
            dmarc: DmarcAuthConfig {
                verify: self
                    .parse_if_block("auth.dmarc.verify", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(VerifyStrategy::Relaxed)),
                report_aggregate: self
                    ...
                    .unwrap_or_default(),
                report_send: self
                    .parse_if_block("auth.dmarc.reporting.send-rate", ctx, &envelope_keys)?
                    .unwrap_or_default(),
                report_analyze: self
                    .parse_if_block("auth.dmarc.reporting.analyze", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(true)),
            },
            tls: TlsAuthConfig {
                report_send: self
                    .parse_if_block("auth.tls.reporting.send-rate", ctx, &envelope_keys)?
                    .unwrap_or_default(),
                report_analyze: self
                    .parse_if_block("auth.tls.reporting.analyze", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(true)),
            },
            iprev: IpRevAuthConfig {
                verify: self
                    .parse_if_block("auth.iprev.verify", ctx, &envelope_keys)?
                    .unwrap_or_else(|| IfBlock::new(VerifyStrategy::Relaxed)),
            },
        })
    }
