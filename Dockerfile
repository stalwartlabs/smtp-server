...
RUN echo "#\!/bin/sh\n\n/usr/local/bin/stalwart-install -c smtp -p /opt/stalwart-smtp -d" > /usr/local/bin/configure.sh && \
    chmod +x /usr/local/bin/configure.sh
RUN useradd stalwart-smtp -s /sbin/nologin -M
RUN mkdir -p /opt/stalwart-smtp
RUN chown stalwart-smtp:stalwart-smtp /opt/stalwart-smtp

VOLUME [ "/opt/stalwart-smtp" ]

ENTRYPOINT ["/usr/local/bin/stalwart-smtp", "--config", "/opt/stalwart-smtp/etc/config.toml"]
